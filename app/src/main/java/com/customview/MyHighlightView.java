...
public class MyHighlightView implements EditableDrawable.OnSizeChange {
    ...
    public int getHit(float x, float y) {
        ...
        if ((mRotateEnabled || mScaleEnabled) && (Math.abs(rect.right - x) < HIT_TOLERANCE) && (Math.abs(rect.bottom - y) < HIT_TOLERANCE) && verticalCheck && horizCheck) {
            retval = ROTATE;
        }
    ...
    public void setup(final Context context, final Matrix m, final Rect imageRect, final RectF cropRect, final boolean maintainAspectRatio, final float rotation) {
        mMatrix = new Matrix(m);
        mRotation = rotation;
        mRotateMatrix = new Matrix();
        outlinePaint.setStyle(Paint.Style.STROKE);
