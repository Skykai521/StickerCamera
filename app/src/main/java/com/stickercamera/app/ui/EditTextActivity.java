...
public class EditTextActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (StringUtils.isNotEmpty(defaultStr)) {
            ...
            if (defaultStr.length() <= maxlength) {
                numberTips.setText("你还可以输入" + (maxlength - defaultStr.length()) + "个字  (" + defaultStr.length() + "/" + maxlength + ")");
            }
        }
    ...
    TextWatcher mTextWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            ...
            numberTips.setText("你还可以输入" + (maxlength - s.toString().length()) + "个字  (" + s.toString().length() + "/" + maxlength + ")");
        }
    };
}
